 ...
 * <p>
 * Обрабатывает HTTP-запросы, связанные с управлением наличием инвентаря в пунктах проката
 * ({@link AvailableEquipment}).
 * Предоставляет пользовательский интерфейс для выполнения CRUD-операций над связями
 * "пункт проката - тип инвентаря".
 * Отображает статистику по доступному инвентарю и обеспечивает выбор пунктов проката
 * и типов инвентаря из выпадающих списков.
 * </p>
 * <p>
...
public class AvailableEquipmentController {
  ...
  public String listAvailableEquipments(Model model) {
    ...
    model.addAttribute("availableEquipmentCount",
      availableEquipmentService.getAvailableEquipmentsCountWithStock());
   ...
   */
  @PostMapping
  public String createAvailableEquipment(@Valid @ModelAttribute("availableEquipment")
   ...
   * Проверяет уникальность новой комбинации "пункт проката + тип инвентаря",
   * если пункт или тип были изменены.
  ...
  public String updateAvailableEquipment(@PathVariable("id") Long id,
                                         @Valid @ModelAttribute("availableEquipment")
                                         AvailableEquipment availableEquipment,
                                         BindingResult result,
                                         RedirectAttributes redirectAttributes,
                                         Model model) {
  ...
  public String deleteAvailableEquipment(@PathVariable("id") Long id,
                                         RedirectAttributes redirectAttributes) {
    try {
      AvailableEquipment availableEquipment = availableEquipmentService.getAvailableEquipmentById(id)
        .orElse(null);
   ...
   */
  @GetMapping("/view/{id}")
  public String viewAvailableEquipment(@PathVariable("id") Long id, Model model,
                                       RedirectAttributes redirectAttributes) {
    AvailableEquipment availableEquipment = availableEquipmentService.getAvailableEquipmentById(id)
      .orElse(null);
