...
public class RentalPointController {
  ...
  public String deleteRentalPoint(@PathVariable("id") Long id,
                                  RedirectAttributes redirectAttributes) {
    try {
      RentalPoint rentalPoint = rentalPointService.getRentalPointById(id)
        .orElse(null);
      ...
      if (rentalPoint != null) {
        ...
        redirectAttributes.addFlashAttribute("successMessage",
          "Пункт проката " + rentalPoint.getPointName() + " успешно удален.");
        return "redirect:/rentalPoints";
      } else {
        redirectAttributes.addFlashAttribute("errorMessage", "Пункт проката не найден.");
      }
   ...
   */
  @GetMapping("/view/{id}")
  public String viewRentalPoint(@PathVariable("id") Long id, Model model,
                                RedirectAttributes redirectAttributes) {
    RentalPoint rentalPoint = rentalPointService.getRentalPointById(id)
      .orElse(null);
